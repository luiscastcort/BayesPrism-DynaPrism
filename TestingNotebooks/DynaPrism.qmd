...
all_types <- unique(as.vector(sce$cellType_broad_hc))
all_states <- unique(as.vector(sce$cellType_hc))

type_to_state <- lapply(all_types,function(t)unique(as.vector(sce$cellType_hc[sce$cellType_broad_hc==t])))
...
#tOmega_s <- readRDS(paste0(data_dir, "NotebookRData/HuukiResiduals/", "Omega_s.rds"))
...
if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "lib_s_exp.rds"))){
  lib_s_exp <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "lib_s_exp.rds"))
} else {
  lib_s_exp <- list()
  lib_s_exp$`Ground Truth` <- sc_exp_tensor(assay(sce, "counts"), colData(sce)$Sample, colData(sce)$cellType_hc, fun = "sum")
}

counts_per_state <- apply(lib_s_exp$`Ground Truth`, c(1,3), sum)
#true_s_frac <- as.table((counts_per_state/rowSums(counts_per_state))[, all_states])
```

```{r}
#| label: state_markers

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "state_markers.rds"))){
  state_markers <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "state_markers.rds"))
} else {
  ...
  saveRDS(state_markers, file=paste0(data_dir, "NotebookRData/DynaPrism/", "state_markers.rds"))
  rm(bp_param)
}
```
...
if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "lib_t_exp.rds"))){
  lib_t_exp <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "lib_t_exp.rds"))
} else {
  lib_t_exp <- list()
  lib_t_exp$`Ground Truth` <- sc_exp_tensor(assay(sce, "counts"), colData(sce)$Sample, colData(sce)$cellType_hc, fun = "sum")
}

counts_per_type <- apply(lib_t_exp$`Ground Truth`, c(1,3), sum)
#true_t_frac <- as.matrix((counts_per_type/rowSums(counts_per_type))[,all_types])
```

```{r}
#| label: type_markers

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "type_markers.rds"))){
  type_markers <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "type_markers.rds"))
} else {
  bp_param <- SnowParam(workers = 14, type = "SOCK")
  ...
  saveRDS(type_markers, file=paste0(data_dir, "NotebookRData/DynaPrism/", "type_markers.rds"))
  rm(bp_param)
}
```
...
tryCatch({
  dp_res <- run.prism.Omega(prism = myPrism, Omega = Omega, n.cores=14, gibbs.control = c(beta = beta))
  saveRDS(dp_res, file=paste0(data_dir, "NotebookRData/DynaPrism/", "dp_res_cBeta050.rds"))
  POST(paste0("https://ntfy.sh/", ntfy_topic),
       body = "Saved dp_res successfully ✅",
       add_headers(Title = "R Process Success")
       )
}, error = function(e) {
  POST(paste0("https://ntfy.sh/", ntfy_topic),
       body = paste0("Getting dp_res failed: ", e$message),
...
#| label: load_dp
#| eval: false
dp_res <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "dp_res_cBeta050.rds")) 
```

### 4.2.2 Pseudobulk

```{r}
pseudobulk <- apply(lib_s_exp$`Ground Truth`, c(1,2), sum)
```
...
tryCatch({
  dp_pseudobulk <- run.prism.Omega(prism = pseudobulk_Prism, Omega = Omega, n.cores=14, gibbs.control = c(beta = beta))
  saveRDS(dp_pseudobulk, file=paste0(data_dir, "NotebookRData/HuukiResiduals/", "dp_pseudobulk_cBeta050.rds"))
  POST(paste0("https://ntfy.sh/", ntfy_topic),
       body = "Saved dp_pseudobulk successfully ✅",
       add_headers(Title = "R Process Success")
       )
}, error = function(e) {
  POST(paste0("https://ntfy.sh/", ntfy_topic),
       body = paste0("Getting dp_pseudobulk failed: ", e$message),
       add_headers(Title = "R Process ERROR ❌", Priotity = "high")
       )
})
```
...
#| eval: false
dp_pseudobulk <- readRDS(paste0(data_dir, "NotebookRData/HuukiResiduals/", "dp_pseudobulk_cBeta050.rds")) 
```

# 5. Matched Sample Evalluations

## 5.1 Fraction estimations

```{r}
dp_name <- paste0("cDynaPrism (β = ", dp_pseudobulk@control_param$gibbs.control$beta, ")")
```

### 5.1.1 Cell-State

```{r}
if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "full_s_frac.rds"))){
  full_s_frac <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "full_s_frac.rds"))
} else {
  full_s_frac <- list()
}
...
lib_s_frac$`Ground Truth` <- as.table((cells_per_state/rowSums(cells_per_state))[,all_states])
#lib_s_frac$`Ground Truth` <- as.table((counts_per_state/rowSums(counts_per_state))[, all_states])

if (!"BayesPrism" %in% names(full_s_frac)){full_s_frac$BayesPrism <- as.table(bp_res@posterior.initial.cellState@theta)[rownames(blk_n_meta),all_states]}

if (!dp_name %in% names(full_s_frac)){full_s_frac[[dp_name]] <- as.table(dp_res@posterior.initial.cellState@theta)[rownames(blk_n_meta),all_states]}

saveRDS(full_s_frac, file = paste0(data_dir, "NotebookRData/DynaPrism/", "full_s_frac.rds"))

lib_s_frac$BayesPrism <- (full_s_frac$BayesPrism[blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type,])
rownames(lib_s_frac$BayesPrism) <- blk_n_meta[rownames(blk_n_meta) %in% rownames(lib_s_frac$BayesPrism),"Sample"]

lib_s_frac[[dp_name]] <- (full_s_frac[[dp_name]][blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type,])
rownames(lib_s_frac[[dp_name]]) <- blk_n_meta[rownames(blk_n_meta) %in% rownames(lib_s_frac[[dp_name]]),"Sample"]
```

```{r}
#| label: lib_s_eval

lib_s_eval$BayesPrism <- eval_frac(lib_s_frac$BayesPrism, lib_s_frac$`Ground Truth`)
lib_s_eval[[dp_name]] <- eval_frac(lib_s_frac[[dp_name]], lib_s_frac$`Ground Truth`)
```

```{r}
#| label: s_frac_plot_data
true_s_frac_df <- as.data.frame(lib_s_frac$`Ground Truth`)
...
bp_s_frac_df <- as.data.frame(full_s_frac$BayesPrism)
...
dp_s_frac_df <- as.data.frame(full_s_frac[[dp_name]])
...
counts_tab <- table(state_sample_counts$meta$Label)
combined_s_df$n_samples <- as.numeric(counts_tab[match(combined_s_df$cell_state, names(counts_tab))])

rm(true_s_frac_df, bp_s_frac_df, dp_s_frac_df)
```
...
ggplot(combined_s_df, mapping = aes(y = Fraction, x = library_prep, fill = cell_state)) +
  geom_col() +
  #scale_fill_viridis_c(direction = 1, name = "sample per state") +
...
if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "full_t_frac.rds"))){
  full_t_frac <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "full_t_frac.rds"))
} else {
  full_t_frac <- list()
}
lib_t_frac <- list()
...
lib_t_frac$`Ground Truth` <- as.table((cells_per_type/rowSums(cells_per_type))[, all_types])
#lib_t_frac$`Ground Truth` <- as.table((counts_per_type/rowSums(counts_per_type))[,all_types])

if (!"BayesPrism" %in% names(full_t_frac)){
  full_t_frac$BayesPrism <- as.table(bp_res@posterior.theta_f@theta)[rownames(blk_n_meta),all_types]
  #full_t_frac$BayesPrism <- as.table(bp_res@posterior.initial.cellType@theta)[rownames(blk_n_meta), all_types]
}

if (!dp_name %in% names(full_t_frac)){
  full_t_frac[[dp_name]] <- as.table(dp_res@posterior.theta_f@theta)[rownames(blk_n_meta), all_types]
  #full_t_frac[[dp_name]] <- as.table(dp_res@posterior.initial.cellType@theta)[rownames(blk_n_meta), all_types]
}

saveRDS(full_t_frac, file = paste0(data_dir, "NotebookRData/DynaPrism/", "full_t_frac.rds"))

lib_t_frac[[dp_name]] <- full_t_frac[[dp_name]][blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type,]
rownames(lib_t_frac[[dp_name]]) <- blk_n_meta[rownames(blk_n_meta) %in% rownames(lib_t_frac[[dp_name]]),"Sample"]
lib_t_frac$BayesPrism <- full_t_frac$BayesPrism[blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type,]
rownames(lib_t_frac$BayesPrism) <- blk_n_meta[rownames(blk_n_meta) %in% rownames(lib_t_frac$BayesPrism),"Sample"]

lib_t_eval$BayesPrism <- eval_frac(lib_t_frac$BayesPrism, lib_t_frac$`Ground Truth`)
lib_t_eval[[dp_name]] <- eval_frac(lib_t_frac[[dp_name]], lib_t_frac$`Ground Truth`)
```

```{r}
#| label: t_frac_plot
true_t_frac_df <- as.data.frame(lib_t_frac$`Ground Truth`)
...
bp_t_frac_df <- as.data.frame(full_t_frac$BayesPrism)
...
dp_t_frac_df <- as.data.frame(full_t_frac[[dp_name]])
...
rm(true_t_frac_df, bp_t_frac_df, dp_t_frac_df)
```
...
## 5.2 CTSE estimations

### 5.2.1 Cell-state

```{r}
#| label: lib_s_CTSE_initialization

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "full_s_exp.rds"))){
  full_s_exp <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "full_s_exp.rds"))
} else {
  full_s_exp <- list()
}

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "lib_s_CTSE.rds"))){
  lib_s_CTSE <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "lib_s_CTSE.rds"))
} else {
  lib_s_CTSE <- list()
}
```
...
if (!"Plain Bulk" %in% names(full_s_exp)){
  
  G_names <- colnames(X)
  N_names <- rownames(X)
  K_names <- all_states
  
  full_s_exp$`Plain Bulk` <- array(NA, 
                        dim = c(length(N_names), length(G_names), length(K_names)),
                        dimnames = list(N_names, G_names, K_names))
  
  for(k in K_names) {
    full_s_exp$`Plain Bulk`[,,k] <- X
  }
  
  rm(G_names, N_names, K_names)
}

if (!"BayesPrism" %in% names(full_s_exp)){full_s_exp$BayesPrism <- bp_res@posterior.initial.cellState@Z}

if (!dp_name %in% names(full_s_exp)){full_s_exp[[dp_name]] <- dp_res@posterior.initial.cellState@Z}

saveRDS(full_s_exp, file = paste0(data_dir, "NotebookRData/DynaPrism/", "full_s_exp.rds"))
```

```{r}
#| label: subset_lib_s_exp

lib_s_exp$`Plain Bulk` <- full_s_exp$`Plain Bulk`[blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type, , ]
dimnames(lib_s_exp$`Plain Bulk`)[[1]] <- blk_n_meta[rownames(blk_n_meta) %in% dimnames(lib_s_exp$`Plain Bulk`)[[1]],"Sample"]

lib_s_exp$BayesPrism <- full_s_exp$BayesPrism[blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type, , ]
dimnames(lib_s_exp$BayesPrism)[[1]] <- blk_n_meta[rownames(blk_n_meta) %in% dimnames(lib_s_exp$BayesPrism)[[1]],"Sample"]

lib_s_exp[[dp_name]] <- full_s_exp[[dp_name]][blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type, , ]
dimnames(lib_s_exp[[dp_name]])[[1]] <- blk_n_meta[rownames(blk_n_meta) %in% dimnames(lib_s_exp[[dp_name]])[[1]],"Sample"]
```

```{r}
#| label: eval_lib_s_CTSE

if ((!"Plain Bulk" %in% names(lib_s_CTSE)) & ("Plain Bulk" %in% names(lib_s_exp))){lib_s_CTSE$`Plain Bulk` <- eval_CTSE(lib_s_exp$`Plain Bulk`, lib_s_exp$`Ground Truth`, markers_list = state_marker_list)}

if ((!dp_name %in% names(lib_s_CTSE)) & (dp_name %in% names(lib_s_exp))){lib_s_CTSE[[dp_name]] <- eval_CTSE(lib_s_exp[[dp_name]], lib_s_exp$`Ground Truth`, markers_list = state_marker_list)}


if ((!"BayesPrism" %in% names(lib_s_CTSE)) & ("BayesPrism" %in% names(lib_s_exp))){lib_s_CTSE$`BayesPrism` <- eval_CTSE(lib_s_exp$BayesPrism, lib_s_exp$`Ground Truth`, markers_list = state_marker_list)}

if ((!"Ground Truth" %in% names(lib_s_CTSE)) & ("Ground Truth" %in% names(lib_s_exp))){lib_s_CTSE$`Ground Truth` <- eval_CTSE(lib_s_exp$`Ground Truth`, lib_s_exp$`Ground Truth`, markers_list = state_marker_list)}

saveRDS(lib_s_CTSE, file = paste0(data_dir, "NotebookRData/DynaPrism/", "lib_s_CTSE.rds"))
```

```{r}
#| label: plot_CTSE_lib_s
plot_dumbbellCTSE(lib_s_CTSE, modified = dp_name, original = "BayesPrism",
                  subtitle = paste0(lib_prep, "-", lib_type, " RNA-seq library"),
                  group_colors = state_colors)

plot_ExpSpe(lib_s_CTSE[[dp_name]], lib_s_CTSE$`BayesPrism`, gene_pressure, 
            title = "State-Level Expression Specificity",
            subtitle = paste0(dp_name, " on ", lib_prep, "-", lib_type, " RNA-seq library"),
            mod_name = dp_name, ori_name = "BayesPrism")

plot_DeltaCTSE(lib_s_CTSE[[dp_name]], lib_s_CTSE$BayesPrism, 
               subtitle = paste0(lib_prep, "-", lib_type, " RNA-seq library"),
               group_colors = state_colors, 
               mod_name = dp_name, ori_name = "BayesPrism")

plot_DeltaCTSE(lib_s_CTSE[[dp_name]], lib_s_CTSE$`DynaPrism (β = 1)`, 
               subtitle = paste0(lib_prep, "-", lib_type, " RNA-seq library"),
               group_colors = state_colors, 
               mod_name = dp_name, ori_name = "DynaPrism (β = 1)")

plot_ExpSpe(lib_s_CTSE[[dp_name]], lib_s_CTSE$`DynaPrism (β = 1)`, gene_pressure, 
            title = "State-Level Expression Specificity",
            subtitle = paste0(dp_name, " on ", lib_prep, "-", lib_type, " RNA-seq library"),
            mod_name = dp_name, ori_name = "DynaPrism (β = 1)")
```
...
```{r}
#| label: lib_t_CTSE_initialization

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "full_t_exp.rds"))){
  full_t_exp <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "full_t_exp.rds"))
} else {
  full_t_exp <- list()
}

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "lib_t_CTSE.rds"))){
  lib_t_CTSE <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "lib_t_CTSE.rds"))
} else {
  lib_t_CTSE <- list()
}
```
...
X <- as.matrix(myPrism@mixture)
X_cpm <- sweep(X, 1, rowSums(X), "/") * 1e6
if (!"Plain Bulk" %in% names(full_t_exp)){
  
  G_names <- colnames(X)
  N_names <- rownames(X)
  K_names <- all_types
  full_t_exp$`Plain Bulk` <- array(NA, 
                        dim = c(length(N_names), length(G_names), length(K_names)),
                        dimnames = list(N_names, G_names, K_names))
  
  for(k in K_names) {
    full_t_exp$`Plain Bulk`[,,k] <- X
  }
  rm(G_names, N_names, K_names)
}

if (!"BayesPrism" %in% names(full_t_exp)){full_t_exp$BayesPrism <- bp_res@posterior.initial.cellType@Z}

if (!dp_name %in% names(full_t_exp)){full_t_exp[[dp_name]] <- dp_res@posterior.initial.cellType@Z}

saveRDS(full_t_exp, file = paste0(data_dir, "NotebookRData/DynaPrism/", "full_t_exp.rds"))
```

```{r}
#| label: regressed_bulk_lib_t
...
common_n <- intersect(rownames(X_lib), rownames(lib_t_frac$`Ground Truth`))

X_lib <- X_lib[common_n, ]
theta_t <- lib_t_frac$`Ground Truth`[common_n, ]
...
lib_t_exp$`Fraction-Regressed Bulk` <- array(NA,
                   dim = c(length(N_names), length(G_names), length(K_names)),
                   dimnames = list(N_names, G_names, K_names))

for(k in K_names) {
  lib_t_exp$`Fraction-Regressed Bulk`[,,k] <- X_regress_t
}

rm(X_lib, theta_t, beta_hat_t, X_hat_t, mu_g, X_regress_t ,G_names, N_names, K_names)
```

```{r}
#| label: lib_t_subsetting

lib_t_exp$`Plain Bulk` <- full_t_exp$`Plain Bulk`[blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type, , ]
dimnames(lib_t_exp$`Plain Bulk`)[[1]] <- blk_n_meta[rownames(blk_n_meta) %in% dimnames(lib_t_exp$`Plain Bulk`)[[1]],"Sample"]

lib_t_exp$BayesPrism <- full_t_exp$BayesPrism[blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type, , ]
dimnames(lib_t_exp$BayesPrism)[[1]] <- blk_n_meta[rownames(blk_n_meta) %in% dimnames(lib_t_exp$BayesPrism)[[1]],"Sample"]

lib_t_exp[[dp_name]] <- full_t_exp[[dp_name]][blk_n_meta$library_prep == lib_prep & blk_n_meta$library_type == lib_type, , ]
dimnames(lib_t_exp[[dp_name]])[[1]] <- blk_n_meta[rownames(blk_n_meta) %in% dimnames(lib_t_exp[[dp_name]])[[1]],"Sample"]

```

```{r}
#| label: eval_CTSE_lib_t
if ((!"Plain Bulk" %in% names(lib_t_CTSE)) & ("Plain Bulk" %in% names(lib_t_exp))){lib_t_CTSE$`Plain Bulk` <- eval_CTSE(lib_t_exp$`Plain Bulk`, lib_t_exp$`Ground Truth`, markers_list = state_marker_list)}

if ((!"Fraction-Regressed Bulk" %in% names(lib_t_CTSE)) & ("Fraction-Regressed Bulk" %in% names(lib_t_exp))){lib_t_CTSE$`Fraction-Regressed Bulk` <- eval_CTSE(lib_t_exp$`Fraction-Regressed Bulk`, lib_t_exp$`Ground Truth`, markers_list = state_marker_list)}

if ((!dp_name %in% names(lib_t_CTSE)) & (dp_name %in% names(lib_t_exp))){lib_t_CTSE[[dp_name]] <- eval_CTSE(lib_t_exp[[dp_name]], lib_t_exp$`Ground Truth`, markers_list = type_marker_list)}

if ((!"BayesPrism" %in% names(lib_t_CTSE)) & ("BayesPrism" %in% names(lib_t_exp))){lib_t_CTSE$BayesPrism <- eval_CTSE(lib_t_exp$BayesPrism, lib_t_exp$`Ground Truth`, markers_list = type_marker_list)}

if ((!"Ground Truth" %in% names(lib_t_CTSE)) & ("Ground Truth" %in% names(lib_t_exp))){lib_t_CTSE$`Ground Truth` <- eval_CTSE(lib_t_exp$`Ground Truth`, lib_t_exp$`Ground Truth`, markers_list = type_marker_list)}

saveRDS(lib_t_CTSE, file = paste0(data_dir, "NotebookRData/DynaPrism/", "lib_t_CTSE.rds"))
```

```{r}
#| label: plot_CTSE_lib_t
plot_dumbbellCTSE(lib_t_CTSE, modified = dp_name, original = "BayesPrism",
                  subtitle = paste0(lib_prep, "-", lib_type, " RNA-seq library"),
                  group_colors = type_colors)

plot_ExpSpe(lib_t_CTSE[[dp_name]], lib_t_CTSE$BayesPrism, gene_pressure, 
            title = "Type-Level Expression Specificity",
            subtitle = paste0(dp_name, " on ", lib_prep, "-", lib_type, " RNA-seq library"),
            mod_name = dp_name, ori_name = "BayesPrism")

plot_DeltaCTSE(lib_t_CTSE[[dp_name]], lib_t_CTSE$BayesPrism, 
               subtitle = paste0(lib_prep, "-", lib_type, " RNA-seq library"),
               group_colors = type_colors, 
               mod_name = dp_name, ori_name = "BayesPrism")

plot_DeltaCTSE(lib_t_CTSE[[dp_name]], lib_t_CTSE$`DynaPrism (β = 1)`, 
               subtitle = paste0(lib_prep, "-", lib_type, " RNA-seq library"),
               group_colors = state_colors, 
               mod_name = dp_name, ori_name = "DynaPrism (β = 1)")

plot_ExpSpe(lib_t_CTSE[[dp_name]], lib_t_CTSE$`DynaPrism (β = 1)`, gene_pressure, 
            title = "Type-Level Expression Specificity",
            subtitle = paste0(dp_name, " on ", lib_prep, "-", lib_type, " RNA-seq library"),
            mod_name = dp_name, ori_name = "DynaPrism (β = 1)")
```
...
## 6.1 Fraction Estimates

```{r}
if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_frac.rds"))){
  psb_s_frac <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_frac.rds"))
} else {
  psb_s_frac <- list()
}

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_frac.rds"))){
  psb_t_frac <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_frac.rds"))
} else {
  psb_t_frac <- list()
}

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_eval.rds"))){
  psb_s_eval <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_eval.rds"))
} else {
  psb_s_eval <- list()
}

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_eval.rds"))){
  psb_t_eval <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_eval.rds"))
} else {
  psb_t_eval <- list()
}
```
...
```{r}
if (!"BayesPrism" %in% names(psb_s_frac)){psb_s_frac$BayesPrism <- as.table(bp_pseudobulk@posterior.initial.cellState@theta)}
if (!dp_name %in% names(psb_s_frac)){psb_s_frac[[dp_name]] <- as.table(dp_pseudobulk@posterior.initial.cellState@theta)}
saveRDS(psb_s_frac, file = paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_frac.rds"))

if (!"BayesPrism" %in% names(psb_s_eval)){psb_s_eval$BayesPrism <- eval_frac(psb_s_frac$BayesPrism, lib_t_frac$`Ground Truth`)}
if (!dp_name %in% names(psb_s_eval)){psb_s_eval[[dp_name]] <- eval_frac(psb_s_frac[[dp_name]], lib_t_frac$`Ground Truth`)}
saveRDS(psb_s_eval, file = paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_eval.rds"))
```
...
bp_psb_s_frac_df <- as.data.frame(psb_s_frac$BayesPrism)
...
dp_psb_s_frac_df <- as.data.frame(psb_s_frac[[dp_name]])
...
dp_psb_s_frac_df$Source <- dp_name
...
rm(dp_psb_s_frac_df, bp_psb_s_frac_df, true_s_frac_df)
```
...
if (!"BayesPrism" %in% names(psb_t_frac)){
  #psb_t_frac$BayesPrism <- as.table(bp_pseudobulk@posterior.initial.cellType@theta)
  psb_t_frac$BayesPrism <- as.table(bp_pseudobulk@posterior.theta_f@theta)
}

if (!dp_name %in% names(psb_t_frac)){
  #psb_t_frac[[dp_name]] <- as.table(dp_pseudobulk@posterior.initial.cellType@theta)
  psb_t_frac[[dp_name]] <- as.table(dp_pseudobulk@posterior.theta_f@theta)
  }
saveRDS(psb_t_frac, file = paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_frac.rds"))


if (!"BayesPrism" %in% names(psb_t_eval)){psb_t_eval$BayesPrism <- eval_frac(psb_t_frac$BayesPrism, lib_t_frac$`Ground Truth`)}

if (!dp_name %in% names(psb_t_eval)){psb_t_eval[[dp_name]] <- eval_frac(psb_t_frac[[dp_name]], lib_t_frac$`Ground Truth`)}

saveRDS(psb_s_eval, file = paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_eval.rds"))
```
...
bp_psb_t_frac_df <- as.data.frame(psb_t_frac$BayesPrism)
...
dp_psb_t_frac_df <- as.data.frame(psb_t_frac[[dp_name]])
...
dp_psb_t_frac_df$Source <- dp_name
...
rm(dp_psb_t_frac_df, bp_psb_t_frac_df, true_t_frac_df)
```
...
```{r}
#| label: psb_s_CTSE_initialization

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_exp.rds"))){
  psb_s_exp <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_exp.rds"))
} else {
  psb_s_exp <- list()
}

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_exp.rds"))){
  psb_t_exp <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_exp.rds"))
} else {
  psb_t_exp <- list()
}


if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_CTSE.rds"))){
  psb_s_CTSE <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_CTSE.rds"))
} else {
  psb_s_CTSE <- list()
}

if (file.exists(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_CTSE.rds"))){
  psb_t_CTSE <- readRDS(paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_CTSE.rds"))
} else {
  psb_t_CTSE <- list()
}
```
...
if (!"Plain Bulk" %in% names(psb_s_exp)){
  G_names <- colnames(psb_X)
  N_names <- rownames(psb_X)
  K_names <- all_states
  psb_s_exp$`Plain Bulk` <- array(NA, 
                   dim = c(length(N_names), length(G_names), length(K_names)),
                   dimnames = list(N_names, G_names, K_names))
  for(k in K_names) {
    psb_s_exp$`Plain Bulk`[,,k] <- psb_X
    }
  rm(psb_X_cpm, G_names, N_names, K_names)
} 

if (!"BayesPrism" %in% names(psb_s_exp)){psb_s_exp$BayesPrism <- bp_pseudobulk@posterior.initial.cellState@Z}

if (!dp_name %in% names(psb_s_exp)){psb_s_exp[[dp_name]] <- dp_pseudobulk@posterior.initial.cellState@Z}

saveRDS(psb_s_exp, file = paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_exp.rds"))
```

```{r}
#| label: eval_CTSE_psb_s
if ((!"Plain Bulk" %in% names(psb_s_CTSE)) & ("Plain Bulk" %in% names(psb_s_exp))){psb_s_CTSE$`Plain Bulk` <- eval_CTSE(psb_s_exp$`Plain Bulk`, lib_s_exp$`Ground Truth`, markers_list = state_marker_list)}

if ((!"BayesPrism" %in% names(psb_s_CTSE)) & ("BayesPrism" %in% names(psb_s_exp))){psb_s_CTSE$BayesPrism <- eval_CTSE(psb_s_exp$BayesPrism, lib_s_exp$`Ground Truth`, markers_list = state_marker_list)}

if ((!dp_name %in% names(psb_s_CTSE)) & (dp_name %in% names(psb_s_exp))){psb_s_CTSE[[dp_name]] <- eval_CTSE(psb_s_exp[[dp_name]], lib_s_exp$`Ground Truth`, markers_list = state_marker_list)}

if ((!"Ground Truth" %in% names(psb_s_CTSE)) & ("Ground Truth" %in% names(lib_s_exp))){psb_s_CTSE$`Ground Truth` <- eval_CTSE(lib_s_exp$`Ground Truth`, lib_s_exp$`Ground Truth`, markers_list = state_marker_list)}

saveRDS(psb_s_CTSE, file = paste0(data_dir, "NotebookRData/DynaPrism/", "psb_s_CTSE.rds"))
```

```{r}
#| label: plot_CTSE_psb_s
plot_dumbbellCTSE(psb_s_CTSE, modified = dp_name, original = "BayesPrism",
                  subtitle = "Pseudobulk",
                  group_colors = state_colors)

plot_ExpSpe(psb_s_CTSE[[dp_name]], psb_s_CTSE$`BayesPrism`, gene_pressure, 
            title = "State-Level Expression Specificity",
            subtitle = "Pseudobulk",
            mod_name = dp_name, ori_name = "BayesPrism")

plot_DeltaCTSE(psb_s_CTSE[[dp_name]], psb_s_CTSE$BayesPrism,
               subtitle = "Pseudobulk",
               group_colors = state_colors, 
               mod_name = dp_name, ori_name = "Bayes Prism")

plot_DeltaCTSE(psb_s_CTSE[[dp_name]], psb_s_CTSE$`DynaPrism (β = 1)`,
               subtitle = "Pseudobulk",
               group_colors = state_colors, 
               mod_name = dp_name, ori_name = "DynaPrism (β = 1)")
```
...
psb_X <- as.matrix(pseudobulk_Prism@mixture)
psb_X_cpm <- sweep(X, 1, rowSums(X), "/") * 1e6
if (!"Plain Bulk" %in% names(psb_t_exp)){
  G_names <- colnames(psb_X)
  N_names <- rownames(psb_X)
  K_names <- all_types
  
  psb_t_exp$`Plain Bulk` <- array(NA, 
                   dim = c(length(N_names), length(G_names), length(K_names)),
                   dimnames = list(N_names, G_names, K_names))
  for(k in K_names) {
    psb_t_exp$`Plain Bulk`[,,k] <- psb_X
    }
  rm(psb_X_cpm, G_names, N_names, K_names)
}

if (!"Fraction-Regressed Bulk" %in% names(psb_t_exp)){
  common_n <- intersect(rownames(psb_X), rownames(lib_t_frac$`Ground Truth`))
  ...
  psb_t_exp$`Fraction-Regressed Bulk` <- array(NA,
                   dim = c(length(N_names), length(G_names), length(K_names)),
                   dimnames = list(N_names, G_names, K_names))
  for(k in K_names) {
    psb_t_exp$`Fraction-Regressed Bulk`[,,k] <- psb_X_regress_t
  }
  
  rm(common_n, theta_t, psb_beta_hat_t, psb_X_hat_t, psb_X_regress_t, G_names, N_names, K_names)
}

if (!"BayesPrism" %in% names(psb_t_exp)){psb_t_exp$BayesPrism <- bp_pseudobulk@posterior.initial.cellType@Z}

if (!dp_name %in% names(psb_t_exp)){psb_t_exp[[dp_name]] <- dp_pseudobulk@posterior.initial.cellType@Z}

saveRDS(psb_t_exp, file = paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_exp.rds"))
```

```{r}
#| label: eval_CTSE_psb_t

if ((!"Plain Bulk" %in% names(psb_t_CTSE)) & ("Plain Bulk" %in% names(psb_t_exp))){psb_t_CTSE$`Plain Bulk` <- eval_CTSE(psb_t_exp$`Plain Bulk`, lib_t_exp$`Ground Truth`, markers_list = state_marker_list)}

if ((!"Fraction-Regressed Bulk" %in% names(psb_t_CTSE)) & ("Fraction-Regressed Bulk" %in% names(psb_t_exp))){psb_t_CTSE$`Fraction-Regressed Bulk` <- eval_CTSE(psb_t_exp$`Fraction-Regressed Bulk`, lib_t_exp$`Ground Truth`, markers_list = state_marker_list)}

if ((!"BayesPrism" %in% names(psb_t_CTSE)) & ("BayesPrism" %in% names(psb_t_exp))){psb_t_CTSE$BayesPrism <- eval_CTSE(psb_t_exp$BayesPrism, lib_t_exp$`Ground Truth`, markers_list = type_marker_list)}

if ((!dp_name %in% names(psb_t_CTSE)) & (dp_name %in% names(psb_t_exp))){psb_t_CTSE[[dp_name]] <- eval_CTSE(psb_t_exp[[dp_name]], lib_t_exp$`Ground Truth`, markers_list = type_marker_list)}

if ((!"Ground Truth" %in% names(psb_t_CTSE)) & ("Ground Truth" %in% names(lib_t_exp))){psb_t_CTSE$`Ground Truth` <- eval_CTSE(lib_t_exp$`Ground Truth`, lib_t_exp$`Ground Truth`, markers_list = type_marker_list)}

saveRDS(psb_t_CTSE, file = paste0(data_dir, "NotebookRData/DynaPrism/", "psb_t_CTSE.rds"))
```

```{r}
#| label: plot_CTSE_psb_t
plot_dumbbellCTSE(psb_t_CTSE, modified = dp_name, original = "BayesPrism",
                  subtitle =  "Pseudobulk",
                  group_colors = type_colors)

plot_ExpSpe(psb_t_CTSE[[dp_name]], psb_t_CTSE$BayesPrism, gene_pressure, 
            title = "Type-Level Expression Specificity",
            subtitle = paste0(dp_name, " on Pseudobulk"),
            mod_name = dp_name, ori_name = "BayesPrism")

plot_DeltaCTSE(psb_t_CTSE[[dp_name]], psb_t_CTSE$BayesPrism, 
               subtitle = "Pseudobulk",
               group_colors = type_colors, 
               mod_name = dp_name, ori_name = "BayesPrism")

plot_DeltaCTSE(psb_t_CTSE[[dp_name]], psb_t_CTSE$`DynaPrism (β = 1)`, 
               subtitle = "Pseudobulk",
               group_colors = type_colors, 
               mod_name = dp_name, ori_name = "DynaPrism (β = 1)")
```