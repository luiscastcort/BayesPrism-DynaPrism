...
```{r}
if (!dir.exists(paste0(data_dir, "Data_Neftel2019_Brain")) & file.exists(paste0(data_dir,"Data_Neftel2019_Brain.tar.gz"))) {
  untar(paste0(data_dir,"Data_Neftel2019_Brain.tar.gz"), exdir = data_dir)
  print("Decompression complete.")
} else if (dir.exists(paste0(data_dir, "Data_Neftel2019_Brain"))) {
  print("Neftel2019 data has been decompressed.")
} else if (!file.exists(paste0(data_dir,"Data_Neftel2019_Brain.tar.gz"))) {
  print("File 'Data_Neftel2019_Brain.tar.gz' is not in the data directory. It may be downloaded from: https://www.weizmann.ac.il/sites/3CA/brain")
}
```
...
```{r}
counts <- readMM(paste0(data_dir, "Data_Neftel2019_Brain/SmartSeq2/Exp_data_TPM.mtx"))
genes <- fread(paste0(data_dir, "Data_Neftel2019_Brain/SmartSeq2/Genes.txt"), header = FALSE)$V1
cells <- fread(paste0(data_dir, "Data_Neftel2019_Brain/SmartSeq2/Cells.csv"), header = TRUE)
barcodes <- cells[[1]]
...
```{r}
counts <- readMM(paste0(data_dir, "Data_Neftel2019_Brain/10X/Exp_data_UMIcounts.mtx"))
genes <- fread(paste0(data_dir, "Data_Neftel2019_Brain/10X/Genes.txt"), header = FALSE)$V1
cells <- fread(paste0(data_dir,"Data_Neftel2019_Brain/10X/Cells.csv"), header = TRUE)
barcodes <- cells[[1]]
